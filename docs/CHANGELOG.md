...
- **Unknown member diagnostics.** Method calls, property accesses, and constant references that do not exist on the resolved class are underlined with a warning after full resolution (inheritance, traits, virtual members from `@method`/`@property`/`@mixin`). Suppressed when the class declares `__call`, `__callStatic`, or `__get` magic methods, and when the subject type cannot be resolved. PHP's case-insensitive method matching is respected.
