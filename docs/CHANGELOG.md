...
- **Add `#[Override]` code action.** When PHPStan reports `method.missingOverride`, a quickfix inserts `#[Override]` above the method declaration with correct indentation and adds a `use Override;` import when the file declares a namespace. Existing attributes on the method are preserved, and the action is suppressed when the attribute is already present. The diagnostic disappears on the next keystroke without waiting for the next PHPStan run.
- **Extract variable.** Select an expression and extract it into a new local variable assigned just before the enclosing statement (`refactor.extract`). Smart name generation from method calls (`getName()` → `$name`), property access (`->email` → `$email`), static calls (`Carbon::now()` → `$now`), and function calls. Names are deduplicated against existing variables in scope.
- **Inline variable.** Place the cursor on a variable assignment and inline its value into every read site, removing the original assignment (`refactor.inline`). Safety checks reject the refactoring when the variable is reassigned, when a side-effectful RHS would be duplicated across multiple reads, or when compound assignments are involved. Binary and ternary expressions are automatically parenthesised when substituted into a larger expression.
...
- **Extract function/method.** Select one or more complete statements inside a function or method body and extract them into a new function (`refactor.extract`). Variables read inside the selection that were defined earlier become parameters; variables written inside the selection that are read afterwards become return values; variables used only within the selection stay local. When the selection uses `$this`, `self::`, `static::`, or `parent::`, the code is extracted as a private method on the enclosing class. Static methods produce a private static extracted method. Multiple return values use array destructuring. Type hints are inferred from the hover resolution pipeline. When the last selected statement is a `return`, the extracted function keeps the return and the call site passes it through (`return $this->extracted(...)`), with the return type carried from the enclosing function's signature. Guard-clause patterns (early returns that do not end the selection) are supported: bare `return;` guards produce a bool-flag call site (`if (!$this->extracted(...)) return;`), uniform-value guards like `return false;` use the inverse as a sentinel (`if (!$this->extracted(...)) return false;`), and mixed non-null return values use a null sentinel (`$__early = extracted(...); if ($__early !== null) return $__early;`). When all guards return `null` (or bare `return;` in void methods) and the selection also computes a single value used afterwards, the extracted function returns the computed value on success or `null` on guard failure, producing a clean `$var = $this->extracted(...); if ($var === null) return null;` call site (or `if ($var === null) return;` for void methods).
- **Generate getter/setter.** Place the cursor on a property declaration and trigger a code action to generate `getX()`/`setX()` accessor methods. Bool properties use an `is` prefix (`isActive()`). Readonly properties only offer a getter. Static properties generate static methods. Docblock-only types produce `@return`/`@param` tags instead of native type hints. Setters return `$this` for fluent chaining. If a getter or setter already exists, the corresponding action is suppressed.
- **Generate property hooks (PHP 8.4+).** Place the cursor on a property declaration and generate `get` and/or `set` hooks inline on the property. Three code actions are offered: "Generate get hook", "Generate set hook", and "Generate get and set hooks". Readonly and static properties are skipped (PHP 8.4 forbids hooks on both). Interface properties generate abstract hook signatures without bodies. Properties that already have one hook only offer the missing one. Default values are preserved when adding hooks.
