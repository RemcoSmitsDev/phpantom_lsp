...
- **Lazy autoload file indexing.** Composer autoload files (`autoload_files.php` and their `require_once` chains) are now scanned with the lightweight byte-level `find_symbols` pass instead of a full AST parse at startup. Functions, constants, and classes are indexed by name and lazily parsed on first access. This reduces init time and memory usage proportional to the number of autoload files (typically 50-100 fewer full parses at startup). Functions inside `function_exists()` guards are still resolved on demand through a last-resort lazy parse of known autoload file paths.
- **Non-Composer function and constant discovery.** Projects without `composer.json` now get cross-file function completion, go-to-definition, and constant resolution. The workspace scanner extracts standalone `function` declarations, `define()` constants, and top-level `const` statements alongside classes in a single byte-level pass. Discovered symbols are lazily parsed on first access.
- **Monorepo support.** When the workspace root has no `composer.json`, PHPantom discovers subdirectories that are independent Composer projects and processes each through the full Composer pipeline (PSR-4 mappings, classmap, vendor packages, autoload files). Loose PHP files outside subproject directories are discovered by a separate workspace scan. All features (completion, hover, go-to-definition, find references, diagnostics) work across subproject boundaries.
...
- **Gitignore-aware workspace scanning.** All directory walkers (full-scan, PSR-4 scanner, vendor package scanner, and go-to-implementation file collector) now use the `ignore` crate to respect `.gitignore` rules instead of hardcoding directory names (`vendor`, `node_modules`, `target`) to skip. Projects with non-standard directory names that happen to match the old skip list no longer lose files silently.
