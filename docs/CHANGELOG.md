...
- **Hover variable resolution uses the unified type engine.** Hover now resolves variable types through the same pipeline as completion instead of a separate raw-type walker. This improves consistency between hover and completion results and enables all narrowing features (instanceof, assert, custom type guards, in_array) to apply during hover. When the cursor is inside a specific if/else branch, hover shows only the type visible in that branch (e.g. `Lamp` inside the if-body, `Faucet` inside the else-body, `Lamp|Faucet` after the if/else).
...
- **Hover and completion type parity.** The hover type-string engine now falls back to the completion engine's ClassInfo resolver for expressions it cannot resolve on its own, eliminating cases where hover showed a different type than completion. This fixes null-coalesce (`??`) expressions where the left-hand side was an invoked closure or other complex expression: hover would show only the right-hand side type (e.g. `Beta`) while completion correctly resolved the left-hand side (e.g. `Alpha`). The null-coalesce handler also gained a syntactic non-nullable check matching the completion engine, so `clone`, `new`, and literal expressions on the left side of `??` are always recognized as non-nullable even when the type string cannot be fully resolved.
- **Type narrowing with same-named classes from different namespaces.** instanceof narrowing (and other type guard narrowing) now correctly distinguishes classes that share a short name but live in different namespaces (e.g. `Contracts\Provider` vs `Concrete\Provider`). Previously the narrowing compared classes by short name only, so narrowing to `Concrete\Provider` was silently ignored when the variable already held `Contracts\Provider`.
- **Generic substitution through transitive interface chains.** When a class implements an interface that itself extends another generic interface (e.g. `CommandCollection @implements TypedCollection<string, Command>` where `TypedCollection @extends IteratorAggregate<TKey, TValue>`), template parameters are now substituted at each level. Previously the intermediate interface's `@extends` generics were propagated with raw template parameter names (e.g. `T` instead of `Command`), breaking foreach element resolution and other generic-dependent features for multi-level interface hierarchies.
...
- **Self-referential array key assignments no longer crash.** Patterns like `$numbers['price'] = $numbers['price']->add(...)` no longer cause a stack overflow during hover or completion. The resolution pipeline previously lacked a guard preventing re-entry into the same assignment when the cursor fell within its right-hand side.
