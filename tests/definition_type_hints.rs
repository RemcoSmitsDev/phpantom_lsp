...
// ═══════════════════════════════════════════════════════════════════════════
// §19  Word extraction edge cases for type hints
// ═══════════════════════════════════════════════════════════════════════════

/// Verify word extraction works correctly for nullable type `?Order`.
#[test]
fn test_extract_word_nullable_type_hint() {
    let content = "<?php\nfunction process(?Order $order): void {}\n";
    // Cursor on "O" in "?Order"
    let pos = Position {
        line: 1,
        character: 19,
    };
    let word = phpantom_lsp::Backend::extract_word_at_position(content, pos);
    assert_eq!(word.as_deref(), Some("Order"));
}

/// Verify word extraction works correctly for union type `Reader|Stream`.
#[test]
fn test_extract_word_union_first_type() {
    let content = "<?php\nfunction process(Reader|Stream $input): void {}\n";
    // Cursor on "R" in "Reader" (character 17: f=0..n=7 ' '=8 p=9..s=15 '('=16 R=17)
    let pos = Position {
        line: 1,
        character: 17,
    };
    let word = phpantom_lsp::Backend::extract_word_at_position(content, pos);
    assert_eq!(word.as_deref(), Some("Reader"));
}

/// Verify word extraction works for second type in union.
#[test]
fn test_extract_word_union_second_type() {
    let content = "<?php\nfunction process(Reader|Stream $input): void {}\n";
    // Cursor on "S" in "Stream" (R=17..r=22 '|'=23 S=24)
    let pos = Position {
        line: 1,
        character: 24,
    };
    let word = phpantom_lsp::Backend::extract_word_at_position(content, pos);
    assert_eq!(word.as_deref(), Some("Stream"));
}

/// Verify word extraction works for intersection type `Foo&Bar`.
#[test]
fn test_extract_word_intersection_first_type() {
    let content = "<?php\nfunction process(Foo&Bar $x): void {}\n";
    // Cursor on "F" in "Foo" ('('=16 F=17)
    let pos = Position {
        line: 1,
        character: 17,
    };
...
/// Verify word extraction works for second type in intersection.
#[test]
fn test_extract_word_intersection_second_type() {
    let content = "<?php\nfunction process(Foo&Bar $x): void {}\n";
    // Cursor on "B" in "Bar" (F=17 o=18 o=19 '&'=20 B=21)
    let pos = Position {
        line: 1,
        character: 21,
    };
    let word = phpantom_lsp::Backend::extract_word_at_position(content, pos);
    assert_eq!(word.as_deref(), Some("Bar"));
}

/// Verify word extraction inside DNF `(Foo&Bar)|null`.
#[test]
fn test_extract_word_dnf_type() {
    let content = "<?php\nfunction process((Foo&Bar)|null $x): void {}\n";
    // Cursor on "F" in "Foo" inside parentheses ('('=16 '('=17 F=18)
    let pos = Position {
        line: 1,
        character: 18,
    };
...
/// Verify word extraction inside docblock generic `Collection<User>`.
#[test]
fn test_extract_word_docblock_generic_inner() {
    let content = "<?php\n/** @return Collection<User> */\n";
    // Cursor on "U" in "User" ('/'=0 '*'=1 '*'=2 ' '=3 '@'=4 ...'n'=10 ' '=11
    // C=12..n=21 '<'=22 U=23)
    let pos = Position {
        line: 1,
        character: 23,
...
/// Verify word extraction for docblock nullable type `?User`.
#[test]
fn test_extract_word_docblock_nullable() {
    let content = "<?php\n/** @return ?User */\n";
    // Cursor on "U" in "?User"
    let pos = Position {
        line: 1,
